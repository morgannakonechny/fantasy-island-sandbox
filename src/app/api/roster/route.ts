...
async function getLiveRoster(req: NextRequest) {
  ...
  try {
    ...
    const rosterData = await yahooFantasyGet(
      `team/${teamInfo.teamKey}/roster/players/stats`,
      accessToken
    );
    const players = extractRoster(rosterData);

    const res = NextResponse.json({ teamName: teamInfo.teamName, players });
    ...
    console.error("Failed to fetch roster from Yahoo", err);
